...
import io.hummingbird.entity.ColumnEntity;
import io.hummingbird.entity.TableEntity;
...
public class GenUtils {
	...
	public static void generatorCode(Map<String, String> table, List<Map<String, String>> columns,
		...
		mainPath = StringUtils.isBlank(mainPath) ? "io.hummingbird" : mainPath;
		...
		map.put("daoPackage", config.getString("dao.package"));
		map.put("daoModuleName", config.getString("dao.moduleName"));
		map.put("entityPackage", config.getString("entity.package"));
		map.put("entityModuleName", config.getString("entity.moduleName"));
		...
		for (String template : templates) {
			...
			if (template.contains("Entity.java")) {
				packageName = config.getString("entity.package");
				moduleName = config.getString("entity.moduleName");
			}
			if (template.contains("Dao.")) {
				packageName = config.getString("dao.package");
				moduleName = config.getString("dao.moduleName");
			}
			// 渲染模板
			...
			try {
				// 添加到zip
				zip.putNextEntry(
						new ZipEntry(getFileName(template, tableEntity.getClassName(), packageName, moduleName)));
